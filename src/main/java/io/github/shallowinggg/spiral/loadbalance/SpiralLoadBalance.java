 ...
 * {@link io.github.shallowinggg.spiral.spring.ApplicationConfigBeanPostProcessor}.
 * If no found, it will use the original invokers as candidates. Then, it will
 * delegate load balance to the fallback {@link LoadBalance} which set by user.
 * If user not set, it will use default load balance provided by dubbo and thus
 * keeps consistent with the original dubbo implementation.
 * <p>
 ...
 * the url provided by invoker will be override by consumer config. So we should find
 * the original provider url with a little hack (a wrapper class exists in
...
public class SpiralLoadBalance extends AbstractLoadBalance {
    ...
    /**
     * Store the fallback {@link LoadBalance}s for each {@link com.alibaba.dubbo.config.annotation.Reference} use.
     * key: {bean_class_name}#{provider_interface_name}
    ...
    /**
     * The tag used to distinguish the specified instances from others.
     * It is provided by system property {@link SpiralConstant#LB_FALLBACK_PROPERTY spiral.tag}.
     */
    private final String tag;

    public SpiralLoadBalance() {
        this.tag = System.getProperty(SpiralConstant.TAG_PROPERTY);
        this.defaultLoadBalance = ExtensionLoader.getExtensionLoader(LoadBalance.class).getExtension(Constants.DEFAULT_LOADBALANCE);
        ...
        for (String fallBackLB : fallbackLBNames.split(",")) {
            ...
            if (fallBackLBEntry.length == 1) {
                ...
                fallBackLbs.put(interfaceName, ExtensionLoader.getExtensionLoader(LoadBalance.class).getExtension(fallbackLBName));
            }
        }
    ...
    protected <T> Invoker<T> doSelect(List<Invoker<T>> invokers, URL url, Invocation invocation) {
        ...
        for (Invoker<T> invoker : invokers) {
            // a little hack
            // see com.alibaba.dubbo.registry.integration.RegistryDirectory$InvokerDelegate
