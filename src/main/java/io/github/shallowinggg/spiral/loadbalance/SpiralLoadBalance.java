 ...
 * {@link io.github.shallowinggg.spiral.spring.ApplicationConfigBeanPostProcessor}. If no found, it will use the
 * original invokers as candidates. Then, it will delegate load balance to the fallback {@link LoadBalance} which set by
 * user. If user not set, it will use default load balance provided by dubbo and thus keeps consistent with the original
 * dubbo implementation.
 * <p>
 * Note: Spiral changes the dubbo application name when starting and use it to isolate with other environment instances.
 * At meantime, when dubbo consumer makes a invocation, the url provided by invoker will be override by consumer config.
 * So we should find the original provider url with a little hack (a wrapper class exists in
...
public class SpiralLoadBalance extends AbstractLoadBalance {
    ...
    /**
     * Store the fallback {@link LoadBalance}s for each {@link com.alibaba.dubbo.config.annotation.Reference} use. key:
    ...
    /**
     * The tag used to distinguish the specified instances from others. It is provided by system property
     * {@link SpiralConstant#LB_FALLBACK_PROPERTY spiral.tag}.
     */
